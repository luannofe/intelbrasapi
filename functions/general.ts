...
async function* separateChunksByBoundary(
  ...
  // function genericParseStringToObject(data: string) {
  //   const lines = data.replace('\r', '').trim().split('\n');
  //   console.table(lines)
  //   const events = [];
  //   let currentEvent = null;
  
  //   for (const line of lines) {
  //     let [property, value] = line.split('=') as [any, any];
  //     const parts = property.split('.');
  //     const eventIndex = parseInt(parts[0].match(/\d+/)[0]);
  
  //     if (!currentEvent || eventIndex !== currentEvent.index) {
  //       currentEvent = {
  //         index: eventIndex,
  //       };
  ...
  //     let currentObj = currentEvent;
  
  //     for (let i = 1; i < parts.length -1 ; i++) {
  //       const part = parts[i];
  //       const isArrayElement = part.includes('[');
  //       const isArray = isArrayElement && currentObj[part.slice(0, part.indexOf('['))] instanceof Array;
  
  //       if (isArray) {
  //         const arrayName = part.slice(0, part.indexOf('['));
  //         const arrayIndex = parseInt(part.match(/\d+/)[0]);
  //         currentObj[arrayName] = currentObj[arrayName] || [];
  //         currentObj[arrayName][arrayIndex] = currentObj[arrayName][arrayIndex] || {};
